...
class EventTriggerSystem:
    ...
    def from_llm_suggestion(
        ...
        # Validate event type
        valid_types = {v for k, v in vars(EventType).items() if not k.startswith("_")}
        if event_type not in valid_types:
            return None

        secondary_ids = [secondary_sim_id] if secondary_sim_id else []
        consequences = build_consequences(
            event_type, primary_sim_id, secondary_ids, engine, extra={"source": "llm"}
        )
        ...
        if getattr(sim, "_low_energy_ticks", 0) >= 5:
            ...
            events.append(
                LifeEvent.make(
                    ...
                    duration_ticks=15,
                    consequences=c,
                    source="trigger:need_based",
                )
            )
            continue

        # Financial crisis — simoleons critically low
        if sim.simoleons < 25 and random.random() < 0.1:
            _record_fire(sim.sim_id, "need_based", tick)
    ...
    for sim in engine.sims:
        ...
        for other in engine.sims:
            ...
            if (
                ...
                and rel.romance < -5
                and rel.friendship <= -25
                and random.random() < 0.05
            ):
                processed_pairs.add(pair)
                _record_fire(sim.sim_id, "relationship_based", tick)
                c = build_consequences(
                    EventType.BREAKUP, sim.sim_id, [other.sim_id], engine
                )
                events.append(
                    LifeEvent.make(
                        EventType.BREAKUP,
                        ...
                        duration_ticks=25,
                        consequences=c,
                        source="trigger:relationship_based",
                    )
                )
        ...
        if rep >= 60 and random.random() < 0.08:
            ...
            events.append(
                LifeEvent.make(
                    ...
                    duration_ticks=30,
                    consequences=c,
                    source="trigger:reputation_based",
                )
            )
...
_DRAMA_TYPES = [
    ...
    (
        ...
        -0.5,
        lambda a, b: (
            f"An unflattering rumour about {a} spread through their social circle."
        ),
    ),
    (
        1,
...
def _romance_can(pair_key: str, stage: str, tick: int) -> bool:
    return (
        tick - _romance_last.get(pair_key, {}).get(stage, -999)
        >= _ROMANCE_COOLDOWNS[stage]
    )
    ...
    for sim in engine.sims:
        for other in engine.sims:
            ...
            if rel.romance >= 20 and _romance_can(pk, "crush", tick):
                ...
                c = build_consequences(
                    EventType.CRUSH_FORMED, sim.sim_id, [other.sim_id], engine
                )
                events.append(
                    LifeEvent.make(
                        EventType.CRUSH_FORMED,
                        sim.sim_id,
            ...
            if (
                rel.romance >= 40
                and rel.interactions >= 6
                and _romance_can(pk, "confess", tick)
            ):
                _romance_record(pk, "confess", tick)
                c = build_consequences(
                    EventType.LOVE_CONFESSION, sim.sim_id, [other.sim_id], engine
                )
                events.append(
                    LifeEvent.make(
            ...
            if rel.romance >= 55 and _romance_can(pk, "kiss", tick):
                ...
                c = build_consequences(
                    EventType.FIRST_KISS_EVENT, sim.sim_id, [other.sim_id], engine
                )
                events.append(
                    LifeEvent.make(
                        EventType.FIRST_KISS_EVENT,
                        sim.sim_id,
            ...
            if rel.romance >= 65 and _romance_can(pk, "dating", tick):
                ...
                c = build_consequences(
                    EventType.DATING_STARTED, sim.sim_id, [other.sim_id], engine
                )
                events.append(
                    LifeEvent.make(
                        EventType.DATING_STARTED,
                        sim.sim_id,
            ...
            if (
                ...
                c = build_consequences(
                    EventType.ENGAGEMENT, sim.sim_id, [other.sim_id], engine
                )
                events.append(
                    LifeEvent.make(
...
_friend_milestones: dict[
    str, set[str]
] = {}  # sim_id → set of "best_friend:other_id" etc.
...
def _check_friendship_arc(engine: "SimEngine", tick: int) -> list[LifeEvent]:
    ...
    for sim in engine.sims:
        for other in engine.sims:
            ...
            # Best friends formed
            if rel.friendship >= 80 and mkey not in _friend_milestones.get(
                sim.sim_id, set()
            ):
                _friend_milestones.setdefault(sim.sim_id, set()).add(mkey)
                _friend_milestones.setdefault(other.sim_id, set()).add(mkey)
                c = build_consequences(
                    EventType.BEST_FRIENDS_FORMED, sim.sim_id, [other.sim_id], engine
                )
                events.append(
                    LifeEvent.make(
                        EventType.BEST_FRIENDS_FORMED,
                        sim.sim_id,
            ...
            if (
                ...
                c = build_consequences(
                    EventType.DRIFTING_APART, sim.sim_id, [other.sim_id], engine
                )
                events.append(
                    LifeEvent.make(
                        EventType.DRIFTING_APART,
                        sim.sim_id,
            ...
            if rel.jealousy_score >= 65 and _can_fire(
                sim.sim_id, "relationship_based", tick
            ):
                _record_fire(sim.sim_id, "relationship_based", tick)
                c = build_consequences(
                    EventType.JEALOUSY_INCIDENT, sim.sim_id, [other.sim_id], engine
                )
                events.append(
                    LifeEvent.make(
                        EventType.JEALOUSY_INCIDENT,
                        sim.sim_id,
    ...
    for sim in engine.sims:
        ...
        if sim.is_child_of:
            ...
            if siblings and random.random() < 0.08:
                ...
                sib = siblings[0]
                c = build_consequences(
                    EventType.SIBLING_RIVALRY, sim.sim_id, [sib.sim_id], engine
                )
                events.append(
                    LifeEvent.make(
                        EventType.SIBLING_RIVALRY,
                        sim.sim_id,
                        f"{sim.name} and {sib.name} are caught in ongoing sibling rivalry.",
                        tick,
                        secondary_sim_ids=[sib.sim_id],
                        visibility=Visibility.HOUSEHOLD,
                        valence=-0.5,
                        intensity=0.6,
                        duration_ticks=15,
                        consequences=c,
                        source="trigger:family_arc",
                    )
                )
        ...
        if hh_members and random.random() < 0.06:
            ...
            other = hh_members[0]
            c = build_consequences(
                EventType.FAMILY_FEUD, sim.sim_id, [other.sim_id], engine
            )
            events.append(
                LifeEvent.make(
                    EventType.FAMILY_FEUD,
                    sim.sim_id,
...
_rumour_pool: list[
    dict
] = []  # {subject_id, text, credibility, believers, tick_created}
    ...
    if random.random() < 0.02 and len(engine.sims) >= 3:
        ...
        if subjects:
            ...
            if tick - _rumour_cooldown.get(creator.sim_id, -80) >= 80:
                ...
                c = build_consequences(
                    EventType.RUMOUR_CREATED, creator.sim_id, [subject.sim_id], engine
                )
                events.append(
                    LifeEvent.make(
                        EventType.RUMOUR_CREATED,
                        creator.sim_id,
                        f'{creator.name} started a rumour: "{rumour_text[:60]}"',
                        tick,
                        secondary_sim_ids=[subject.sim_id],
                        visibility=Visibility.CLUB,
                        valence=-0.3,
                        intensity=0.4,
                        duration_ticks=20,
                        consequences=c,
                        source="trigger:gossip",
                    )
                )
    ...
    for rumour in list(_rumour_pool):
        ...
        if random.random() < 0.08:
            ...
            if potential:
                ...
                believe_chance = rumour["credibility"] * (
                    1.0 - hearer.ocean.get("openness", 0.5) * 0.3
                )
                if (
                    random.random() < believe_chance
                    and hearer.sim_id != rumour["subject_id"]
                ):
                    subject = engine._sim_lookup.get(rumour["subject_id"])
                    if subject:
                        c = build_consequences(
                            EventType.RUMOUR_BELIEVED,
                            hearer.sim_id,
                            [rumour["subject_id"]],
                            engine,
                            extra={
                                "subject_id": rumour["subject_id"],
                        ...
                        events.append(
                            LifeEvent.make(
                                ...
                                tick,
                                secondary_sim_ids=[rumour["subject_id"]],
                                visibility=Visibility.PRIVATE,
                                valence=-0.4,
                                intensity=0.5,
                                duration_ticks=15,
...
def _check_random_drama(engine: "SimEngine", tick: int) -> list[LifeEvent]:
    ...
    active = [
        ...
        if not getattr(s, "_sleeping", False) and s.reputation_score > -80
    ]
...
def _check_aging_arc(engine, tick):
    ...
    for sim in engine.sims:
        ...
        if skey not in fired:
            fired.add(skey)
            c = build_consequences(
                EventType.LIFE_STAGE_TRANSITION,
                sim.sim_id,
                [],
                engine,
            ...
            events.append(
                LifeEvent.make(
                    ...
                    duration_ticks=20,
                    consequences=c,
                    source="trigger:aging",
                )
            )

        if (
            40 <= age <= 55
            and sim.career_performance < 45
            and "midlife_crisis" not in fired
        ):
            if random.random() < 0.08:
...
def _check_career_depth(engine, tick):
    ...
    for sim in engine.sims:
        ...
        for cw_id in sim.coworker_ids:
            ...
            if (
                rel.romance >= 30
                and rel.friendship >= 40
                and pk not in f_set
                and random.random() < 0.15
            ):
                f_set.add(pk)
                c = build_consequences(
                    EventType.WORKPLACE_ROMANCE, sim.sim_id, [cw_id], engine
                )
                events.append(
                    LifeEvent.make(
                        EventType.WORKPLACE_ROMANCE,
                        sim.sim_id,
                        f"{sim.name} and {cw.name} have a workplace romance brewing.",
                        tick,
                        secondary_sim_ids=[cw_id],
                        visibility=Visibility.CLUB,
                        valence=+0.6,
                        intensity=0.6,
                        duration_ticks=20,
            ...
            if rel.friendship < -20 and rk not in f_set and random.random() < 0.12:
                f_set.add(rk)
                c = build_consequences(
                    EventType.WORKPLACE_RIVALRY, sim.sim_id, [cw_id], engine
                )
                events.append(
                    LifeEvent.make(
                        EventType.WORKPLACE_RIVALRY,
                        sim.sim_id,
...
def _check_education(engine, tick):
    ...
    events = []
    children = [
        s for s in engine.sims if s.is_child_of and s.profile.get("age", 25) < 18
    ]
    ...
    for sim in children:
        ...
        if logic >= 8 and skey not in _aging_fired.get(sim.sim_id, set()):
            ...
            c = build_consequences(
                EventType.SCHOLARSHIP, sim.sim_id, [], engine, extra={"amount": 2000.0}
            )
            events.append(
                LifeEvent.make(
    ...
    for sim in engine.sims:
        ...
        if sim.profile.get("age", 0) == 18 and gkey not in _aging_fired.get(
            sim.sim_id, set()
        ):
...
def _check_health_depth(engine, tick):
    ...
    for sim in engine.sims:
        ...
        if getattr(sim, "_high_perf_low_energy_ticks", 0) >= 8 and _can_fire(
            sim.sim_id, "need_based", tick
        ):
            _record_fire(sim.sim_id, "need_based", tick)
            c = build_consequences(EventType.CHRONIC_STRESS, sim.sim_id, [], engine)
            events.append(
                LifeEvent.make(
                    EventType.CHRONIC_STRESS,
                    sim.sim_id,
        ...
        if random.random() < 0.005 and _can_fire(sim.sim_id, "need_based", tick):
            ...
            c = build_consequences(
                EventType.INJURY, sim.sim_id, [], engine, extra={"severity": severity}
            )
            events.append(
                LifeEvent.make(
                    EventType.INJURY,
        ...
        if (
            getattr(sim, "health_status", "healthy") == "sick"
...
def _check_world_context(engine, tick):
    ...
    if tick - last >= 30:
        ...
        if weather_name == "heatwave":
            ...
            events.append(
                LifeEvent.make(
                    EventType.HEATWAVE_EVENT,
                    engine.sims[0].sim_id if engine.sims else "",
    ...
    for sim in engine.sims:
        if weather_name in ("cloudy", "foggy", "rainy"):
            if (
                ...
                _record_fire(sim.sim_id, "need_based", tick)
                c = build_consequences(
                    EventType.SEASONAL_DEPRESSION, sim.sim_id, [], engine
                )
                events.append(
                    LifeEvent.make(
                        EventType.SEASONAL_DEPRESSION,
                        sim.sim_id,
...
def _check_community(engine, tick):
    ...
    fest_key = "festival"
    if (
        tick - _community_cooldowns.get(fest_key, -40) >= 40
        ...
        for sim in engine.sims:
            for other in engine.sims:
                ...
                if rel.friendship <= -50 and random.random() < 0.08:
                    ...
                    c = build_consequences(
                        EventType.NEIGHBORHOOD_DISPUTE,
                        sim.sim_id,
                        [other.sim_id],
                        engine,
                    )
...
def _check_household(engine, tick):
    ...
    for hh in engine.households:
        members = [
            engine._sim_lookup[mid]
            for mid in hh.member_ids
            if mid in engine._sim_lookup
        ]
        if not members:
            continue
        ...
        if (
            hh.funds < 0
            and tick - _household_cooldowns.get(hh_key + "_bills", -60) >= 60
            and random.random() < 0.08
        ):
            _household_cooldowns[hh_key + "_bills"] = tick
            primary = members[0].sim_id
            c = build_consequences(
                EventType.BILLS_CRISIS, primary, [m.sim_id for m in members[1:]], engine
            )
            events.append(
                LifeEvent.make(
                    EventType.BILLS_CRISIS,
                    primary,
                    f"The {hh.name} household can't pay the bills — financial crisis.",
                    tick,
                    secondary_sim_ids=[m.sim_id for m in members[1:]],
                    visibility=Visibility.HOUSEHOLD,
                    valence=-0.7,
                    intensity=0.8,
                    duration_ticks=15,
                    consequences=c,
                    source="trigger:household",
                )
            )
        ...
        if (
            ...
            primary = members[0].sim_id
            c = build_consequences(
                EventType.EVICTION_RISK,
                primary,
                [m.sim_id for m in members[1:]],
                engine,
            )
            events.append(
                LifeEvent.make(
            ...
            for i, ma in enumerate(members):
                for mb in members[i + 1 :]:
                    ...
                    if (
                        ...
                        c = build_consequences(
                            EventType.ROOMMATE_CONFLICT, ma.sim_id, [mb.sim_id], engine
                        )
                        events.append(
                            LifeEvent.make(
                                EventType.ROOMMATE_CONFLICT,
                                ma.sim_id,
                                f"{ma.name} and {mb.name} have a serious roommate conflict.",
                                tick,
                                secondary_sim_ids=[mb.sim_id],
                                visibility=Visibility.HOUSEHOLD,
                                valence=-0.6,
                                intensity=0.7,
                                duration_ticks=15,
